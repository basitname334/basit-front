            ...
            <div className="flex items-center gap-4 sm:gap-6 flex-1">
              ...
              {auth?.role === 'admin' && (
                <nav className="hidden lg:flex items-center gap-1">
                  ...
                  >
                    <span className="relative z-10 flex items-center gap-1.5"><MdAdminPanelSettings className="text-base" /> {t('admin')}</span>
                  ...
                  <Link 
                    ...
                    className="px-4 py-2 text-sm font-semibold text-gray-700 rounded-xl hover:bg-gradient-to-r hover:from-blue-50 hover:to-indigo-50 hover:text-blue-700 transition-all duration-200 relative group"
                  >
                    <span className="relative z-10 flex items-center gap-1.5"><MdDashboard className="text-base" /> {t('dashboard')}</span>
                    <span className="absolute inset-0 rounded-xl bg-gradient-to-r from-blue-100 to-indigo-100 opacity-0 group-hover:opacity-100 transition-opacity"></span>
                  </Link>
                  <Link 
                    to="/orders" 
                    className="px-4 py-2 text-sm font-semibold text-gray-700 rounded-xl hover:bg-gradient-to-r hover:from-blue-50 hover:to-indigo-50 hover:text-blue-700 transition-all duration-200 relative group"
                  >
                    <span className="relative z-10 flex items-center gap-1.5"><MdReceipt className="text-base" /> {t('orders')}</span>
                    <span className="absolute inset-0 rounded-xl bg-gradient-to-r from-blue-100 to-indigo-100 opacity-0 group-hover:opacity-100 transition-opacity"></span>
                  </Link>
                  <Link 
                    to="/reports" 
                    className="px-4 py-2 text-sm font-semibold text-gray-700 rounded-xl hover:bg-gradient-to-r hover:from-blue-50 hover:to-indigo-50 hover:text-blue-700 transition-all duration-200 relative group"
                  >
                    <span className="relative z-10 flex items-center gap-1.5"><MdBarChart className="text-base" /> {t('reports')}</span>
                    <span className="absolute inset-0 rounded-xl bg-gradient-to-r from-blue-100 to-indigo-100 opacity-0 group-hover:opacity-100 transition-opacity"></span>
                  </Link>
                </nav>
              )}
              ...
              {auth?.role === 'user' && (
                <nav className="hidden lg:flex items-center gap-1">
                  <Link 
                    ...
                    className="px-4 py-2 text-sm font-semibold text-gray-700 rounded-xl hover:bg-gradient-to-r hover:from-blue-50 hover:to-indigo-50 hover:text-blue-700 transition-all duration-200 relative group"
                  >
                    <span className="relative z-10 flex items-center gap-1.5"><MdShoppingCart className="text-base" /> {t('newOrder')}</span>
                    <span className="absolute inset-0 rounded-xl bg-gradient-to-r from-blue-100 to-indigo-100 opacity-0 group-hover:opacity-100 transition-opacity"></span>
                  </Link>
                  <Link 
                    to="/dashboard" 
              ...
              {auth && (
                <div className="hidden sm:flex items-center gap-2 text-xs sm:text-sm text-gray-600">
                  <span className="px-3 py-1.5 bg-gradient-to-r from-gray-50 to-gray-100 border border-gray-200 rounded-full font-medium shadow-sm">
                    {auth.role === 'admin' ? <MdAdminPanelSettings className="text-base" /> : <MdPerson className="text-base" />} <span className="hidden md:inline">{auth.email}</span>
                  </span>
                </div>
              )}
              {auth ? (
                ...
                >
                  <span className="hidden sm:inline"><MdExitToApp className="inline text-base" /> </span>{t('logout')}
                </button>
              ) : (
                <Link 
                  to="/login" 
            ...
            <div className="px-4 py-3 space-y-1">
              {auth.role === 'admin' && (
                <>
                  ...
                  >
                    <MdAdminPanelSettings className="text-xl" /> <span>{t('admin')}</span>
                  </Link>
                  <Link 
                    to="/dashboard" 
                    onClick={() => setMobileMenuOpen(false)}
                    className="flex items-center gap-3 px-4 py-3 text-sm font-semibold text-gray-700 rounded-xl hover:bg-gradient-to-r hover:from-blue-50 hover:to-indigo-50 hover:text-blue-700 transition-all duration-200"
                  >
                    <MdDashboard className="text-xl" /> <span>{t('dashboard')}</span>
                  </Link>
                  <Link 
                    to="/orders" 
                    onClick={() => setMobileMenuOpen(false)}
                    className="flex items-center gap-3 px-4 py-3 text-sm font-semibold text-gray-700 rounded-xl hover:bg-gradient-to-r hover:from-blue-50 hover:to-indigo-50 hover:text-blue-700 transition-all duration-200"
                  >
                    <MdReceipt className="text-xl" /> <span>{t('orders')}</span>
                  </Link>
                  <Link 
                    to="/reports" 
                    onClick={() => setMobileMenuOpen(false)}
                    className="flex items-center gap-3 px-4 py-3 text-sm font-semibold text-gray-700 rounded-xl hover:bg-gradient-to-r hover:from-blue-50 hover:to-indigo-50 hover:text-blue-700 transition-all duration-200"
                  >
                    <MdBarChart className="text-xl" /> <span>{t('reports')}</span>
                  </Link>
                </>
              )}
              {auth.role === 'user' && (
                <>
                  <Link 
                    ...
                    className="flex items-center gap-3 px-4 py-3 text-sm font-semibold text-gray-700 rounded-xl hover:bg-gradient-to-r hover:from-blue-50 hover:to-indigo-50 hover:text-blue-700 transition-all duration-200"
                  >
                    <MdShoppingCart className="text-xl" /> <span>{t('newOrder')}</span>
                  </Link>
                  <Link 
                    to="/dashboard" 
                    onClick={() => setMobileMenuOpen(false)}
              ...
              <div className="pt-2 mt-2 border-t border-gray-200">
                <div className="px-4 py-2 text-xs text-gray-500">
                  {auth.role === 'admin' ? <MdAdminPanelSettings className="text-base" /> : <MdPerson className="text-base" />} {auth.email}
                </div>
              </div>
            </div>
          </div>
